...
const TrainerModal = ({ trainer, onClose, onSave, projects, currentRole, successData }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    salutation: trainer ? trainer.salutation : 'Mr.',
    ...
    beneficiaries: trainer?.beneficiaries || ['', '', '', '', '']
  });
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden overflow-y-auto max-h-[95vh] sm:max-h-[90vh]">
        ...
        <form onSubmit={handleSave} className="p-8 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="md:col-span-2 grid grid-cols-6 gap-4">
              <div className="col-span-2">
                <label className="block text-xs font-black text-slate-700 uppercase tracking-widest mb-2">Salutation</label>
                <select
                  value={formData.salutation}
                  onChange={e => setFormData({ ...formData, salutation: e.target.value })}
                  className="w-full h-14 px-4 bg-slate-50 border border-slate-200 rounded-2xl text-slate-900 font-bold focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all"
                >
          ...
          <div className="bg-slate-900 p-8 rounded-[2rem] space-y-6">
            <h4 className="text-[11px] font-black text-blue-400 uppercase tracking-[0.25em] flex items-center gap-3">
              <span className="w-1.5 h-4 bg-blue-500 rounded-full"></span>
              Assigned Beneficiaries (Max 5)
            </h4>
            <div className="grid grid-cols-1 gap-3">
              {formData.beneficiaries.map((ben, idx) => (
                <div key={idx} className="flex items-center gap-4">
                  <span className="text-slate-500 font-black text-[10px] w-4">{idx + 1}.</span>
                  <input
                    value={ben}
                    placeholder="Enter Registration No (e.g. MH124...)"
                    onChange={e => {
                      const newBens = [...formData.beneficiaries];
                      newBens[idx] = e.target.value;
                      setFormData({ ...formData, beneficiaries: newBens });
                    }}
                    className="flex-1 h-12 px-5 bg-white/5 border border-white/10 rounded-xl text-white font-bold text-xs focus:ring-4 focus:ring-blue-500/20 outline-none transition-all placeholder:text-slate-600"
                  />
                </div>
              ))}
            </div>
...
const TrainerManagement = ({ onNavigate, currentRole }) => {
    ...
    try {
      ...
      if (response.data.success) {
        setTrainersList(response.data.data.map(t => ({
          ...t,
          name: t.fullName || 'Unnamed Staff',
          mobile: t.mobileNumber || 'N/A',
          projects: Array.isArray(t.assignedProjects) ? t.assignedProjects : [],
          location: t.district || t.joiningLocation || 'TBD',
  ...
  const handleAction = async (id, role, action) => {
    try {
      const response = await API.put(`/admin/users/${id}/${action}`, { role });
      if (response.data.success) {
        showAlert({ title: 'Success', message: `Trainer ${action}ed successfully`, variant: 'success' });
        fetchTrainers();
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-[2rem] border border-rose-100 text-rose-600 bg-rose-50 p-8 flex items-center gap-6 shadow-sm">
          <div className="w-20 h-20 rounded-2xl bg-white border border-rose-100 flex items-center justify-center text-3xl shadow-sm">❌</div>
          <div>
            <p className="text-[10px] font-black text-slate-700 uppercase tracking-[0.2em] mb-1">Rejected Trainers</p>
            <p className="text-4xl font-black text-slate-900 tracking-tight">{trainersList.filter(t => t.status === 'Rejected').length}</p>
          </div>
        </div>
      </div>
              ...
              <tbody>
                {filtered.map((t, i) => (
                  ...
                  >
                    ...
                    <td className="px-6 sm:px-8 py-6">
                      <div className="flex flex-col gap-1.5 p-3 bg-slate-50 border border-slate-100 rounded-2xl">
                        <div className="flex items-center gap-2">
                          <span className="w-1.5 h-1.5 rounded-full bg-blue-600"></span>
                          <p className="text-[10px] font-black text-slate-900 uppercase tracking-tighter line-clamp-1">
                            {t.placementLocation?.state || 'N/A'}
                          </p>
                        </div>
                        <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest pl-3">
                          {t.placementLocation?.district || 'TBD'} • {t.placementLocation?.taluka || 'TBD'}
                        </p>
                        <div className="bg-white px-2 py-1.5 rounded-lg border border-slate-100 mt-1">
                          <p className="text-[8px] font-extrabold text-blue-600 uppercase tracking-tighter flex items-center gap-1">
                            <span>📍</span> {t.placementLocation?.village || 'TBD'}
                          </p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-8 py-6">
                      <div className="flex gap-2">
                        <button
                          onClick={() => {
                            console.log('Viewing Trainer Detail:', t._id);
                            onNavigate && onNavigate('trainer-detail', { trainerId: t._id });
                          }}
                        ...
                        {currentRole === 'admin' && (
                          <div className="flex gap-2 border-l border-slate-100 pl-4 py-1">
                            <button 
                              onClick={() => handleAction(t._id, 'trainer', 'approve')}
                              className="w-10 h-10 bg-emerald-50 text-emerald-600 hover:bg-emerald-600 hover:text-white rounded-xl flex items-center justify-center transition-all shadow-sm font-black text-lg"
                              title="Approve Name"
                            >✓</button>
                            <button 
                              onClick={() => handleAction(t._id, 'trainer', 'reject')}
                              className="w-10 h-10 bg-rose-50 text-rose-600 hover:bg-rose-600 hover:text-white rounded-xl flex items-center justify-center transition-all shadow-sm font-black text-lg"
                              title="Reject Name"
                            >✕</button>
                          </div>
                        )}
